...
| `GAP-PLAT-003` | `P0` | Durable records | `PARTIAL`, `LOCAL-VERIFIED`: renewal migration, deterministic seed, constraints and scoped repository exist; the graph now drives policy-list renewal rows and linked contact detail while other CRM projections remain browser-owned | Phase 1 | Finish remaining projections, then add browser reload and backup/restore tests |
