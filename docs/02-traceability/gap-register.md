...
| `GAP-PLAT-003` | `P0` | Durable records | `PARTIAL`, `LOCAL-VERIFIED`: renewal migration, deterministic seed, constraints and scoped repository exist; contact GETs, dormant atomic create/receipt persistence, and dormant client create/replay/archive helpers are proven, while browser readers/writes remain authoritative and production POST is unregistered | Phase 1 | Cut all contact readers over atomically, then activate durable creation and extend backup/restore tests |
