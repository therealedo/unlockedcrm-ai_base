...
| Persistence | Browser `localStorage` remains authority for legacy records; policy list, linked contact detail, policy detail, and Renewal Dashboard read the validated synthetic renewal graph from local PostgreSQL without storing it in `CrmData` | Local PostgreSQL, accessed only through workspace-scoped repositories |
