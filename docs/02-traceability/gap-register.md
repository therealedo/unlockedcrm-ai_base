...
| `GAP-PLAT-003` | `P0` | Durable records | `PARTIAL`, `LOCAL-VERIFIED`: renewal migration, deterministic seed, constraints and scoped repository exist; one GET exposes the graph with empty/not-found/error behavior, while mutation and browser authority remain missing | Phase 1 | Command transactions, reload and concurrency tests |
