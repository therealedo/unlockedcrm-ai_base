...
| `CAP-PLAT-003` | PostgreSQL durable data/migrations | `PARTIAL` | Initial migration, deterministic renewal seed, immutable audit and repository tests are `LOCAL-VERIFIED`; no CRM route uses them | Phase 1 development-grade persistence; Phase 2 hardening | GET/command transactions, reload, backup and restore tests |
