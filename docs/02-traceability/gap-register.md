...
| `GAP-PLAT-003` | `P0` | Durable records | `PARTIAL`, `LOCAL-VERIFIED`: renewal migration, deterministic seed, constraints and scoped repository exist; completed/absent tasks no longer hide open renewals or existing task-history audits (`UNIT2B-FIX-2026-09-07`); no CRM route uses them | Phase 1 | GET/command transactions, reload and concurrency tests |
| `GAP-PLAT-004` | `P0` | Workspace ownership seam | `PARTIAL`, `LOCAL-VERIFIED`: one renewal graph has composite scope constraints and repository negatives; hierarchy is `LIVE-VERIFIED` | Phase 1 seam; Phase 3 multi-workspace isolation | Carry centralized workspace context through routes and extend the seam to all records/jobs/files/events/search/exports |
| `GAP-PLAT-005` | `P1` | Events and bounded async workers | `PARTIAL`, `LOCAL-VERIFIED`: immutable renewal creation audit exists; outbox/workers are missing | Phase 1 functional slice; Phase 2 operations | Transactional completion event, outbox/inbox, persisted jobs, idempotency, retry and reconciliation; capacity/dead-letter operations later |
...
| Gap ID | Priority | Capability | Current status/evidence | Target ownership | Exit proof |
|---|---|---|---|---|---|
| `GAP-OPS-001` | `P0` | Reproducible Windows development | `PARTIAL`, `LOCAL-VERIFIED`; pinned Foundation, Compose PostgreSQL, health API, migration, and deterministic seed exist | Phase 1 | Unit 2C adds persistence-aware readiness/startup and clean-machine replay without a cloud account |
