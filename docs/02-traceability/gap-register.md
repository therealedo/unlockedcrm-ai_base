...
| `GAP-PLAT-003` | `P0` | Durable records | `PARTIAL`, `LOCAL-VERIFIED`: renewal migration, deterministic seed, constraints and scoped repository exist; additive contact storage plus workspace-scoped list/detail GETs are proven, while browser readers/writes remain authoritative | Phase 1 | Cut all contact readers over atomically, then activate durable creation and extend backup/restore tests |
