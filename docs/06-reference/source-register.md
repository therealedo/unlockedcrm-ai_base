...
| `CONTACT-WRITE-ARCHIVE-CLIENT-2026-09-09` | Dormant generic contact create/replay and legacy archive helpers | `LOCAL-VERIFIED` | Strict-TDD unit proof for exact create request/response/event/replay validation, operation-scoped errors, and lossless arbitrary legacy-contact archiving while current noncontact edits win; no active caller or runtime responsiveness claim, browser authority unchanged, production POST 404 |
