...
| `GAP-PLAT-003` | `P0` | Durable records | `PARTIAL`, `LOCAL-VERIFIED`: renewal migration, deterministic seed, constraints and scoped repository exist; the graph drives policy list, linked contact detail, policy detail, and Renewal Dashboard while other CRM projections remain browser-owned | Phase 1 | Finish remaining projections, then add browser reload and backup/restore tests |
...
| `GAP-CRM-003` | `P1` | Policies, renewals, dashboards, analytics | Four managed GET reads are `PARTIAL`, `LOCAL-VERIFIED`; Home, Tasks, Analytics/Audit, completion UI, and full policy/renewal workflows remain independent or missing | Phase 1 | Extend the normalized graph to remaining owned views and derived metrics without inventing provider facts |
