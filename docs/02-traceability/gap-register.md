...
| `GAP-PLAT-003` | `P0` | Durable records | `PARTIAL`, `LOCAL-VERIFIED`: renewal migration, deterministic seed, constraints and scoped repository exist; additive contact fields, tags, receipt storage, bounded synthetic seeds, and coexistence-safe replay now form an unwired database foundation; generic contact reads/writes remain browser-owned | Phase 1 | Add contact API/client authority, then extend durable authority and backup/restore tests |
...
| Gap ID | Priority | Capability | Current status/evidence | Target ownership | Exit proof |
|---|---|---|---|---|---|
| `GAP-CRM-001` | `P1` | Contact and household graph | Generic creation remains browser-owned `PARTIAL`; additive PostgreSQL contact/tag/receipt schema and three bounded synthetic seeds are `LOCAL-VERIFIED`, but no generic contact route or UI uses them; complete contact/household lifecycle is `MISSING` | Phase 1 | Wire PostgreSQL-backed create/read authority before edit/archive, relationships, history, and cross-module reload proof |
