...
| `GAP-PLAT-003` | `P0` | Durable records | `PARTIAL`, `LOCAL-VERIFIED`: renewal migration, deterministic seed, constraints and scoped repository exist; completion-aware classification accepts only empty, exact pending, or exact completed fixed graphs, and atomic task completion converges under replay/concurrency while browser authority remains missing | Phase 1 | Browser reload plus backup/restore tests |
