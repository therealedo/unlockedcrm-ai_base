...
> **The current local build is a fictional-data browser prototype plus one local synthetic renewal GET and task-completion POST, not safe for real PII/PHI, hosted production use, or SaaS.**
...
| Persistence | Browser `localStorage` remains authority for legacy records; seven GET-consuming views project the validated PostgreSQL renewal graph across six canonical surfaces (Home, contact detail, policy detail, Renewal Dashboard, Tasks, and Analytics Audit), with policy list as an auxiliary read, without storing the graph in `CrmData` | Local PostgreSQL, accessed only through workspace-scoped repositories |
...
| Data access | Prisma 7.10 plus reviewed constraints, a workspace-scoped repository, and renewal read/completion routes | Prisma plus reviewed, isolated custom SQL escape hatches |
| Async work | Absent | Bounded workers behind durable job/outbox contracts |
| Deployment | `npm run dev` is the Windows-local persistence startup; hosted deployment remains absent | Railway remains an unvalidated Phase 2 candidate and Vercel preview-only |
...
Docker Desktop and Docker Compose are implemented Phase 1 dependencies. `npm run dev` performs the bounded persistence startup and starts the host API/web; `dev:foundation` preserves the generation-independent health shell. The current API exposes a synthetic, workspace-scoped renewal GET and task-completion POST; its renewal read is authoritative on the six canonical surfaces, while unrelated legacy records and mutations remain browser-owned. Object storage, mail capture, queues, and other services join only when a functional slice needs them; full application containerization remains deferred unless measured environment-parity problems justify it.
