...
| `CAP-PLAT-002` | Authorization context, fixed roles and record ownership | `PARTIAL` | Server-controlled synthetic workspace/actor context gates the renewal GET (`LOCAL-VERIFIED`); roles and production identity are absent | Phase 1 centralized synthetic policy context; Phase 2 fixed-role enforcement | Extend ownership scope, then prove production negative authorization |
| `CAP-PLAT-003` | PostgreSQL durable data/migrations | `PARTIAL` | Initial migration, deterministic renewal seed, immutable audit and repository tests are `LOCAL-VERIFIED`; completion-aware startup accepts only empty, exact pending, or exact completed fixed graphs, while command and browser authority remain missing | Phase 1 development-grade persistence; Phase 2 hardening | Command transactions, reload, backup and restore tests |
...
| `CAP-PLAT-008` | Audit/analytics event store | `PARTIAL` | Fixed renewal creation audit, database immutability, and GET projection with persisted actor/record/correlation/provenance metadata are `LOCAL-VERIFIED`; general query and retention remain missing | Phase 1 functional local events; Phase 2 hardening | Atomic completion append, query and retention proof |
...
| `CAP-PLAT-013` | Fastify Node.js TypeScript modular-monolith REST API | `PARTIAL` | First workspace-scoped renewal GET and persistence readiness are `LOCAL-VERIFIED`; Foundation mode remains generation-independent and health-only | Phase 1 | Add commands and remaining domain routes behind repositories |
