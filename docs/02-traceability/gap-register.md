...
| `GAP-PLAT-003` | `P0` | Durable records | `PARTIAL`, `LOCAL-VERIFIED`: renewal migration, deterministic seed, constraints and scoped repository exist; the renewal graph supports durable completion/refetch across six canonical surfaces; unrelated records and mutations remain browser-owned | Phase 1 | Extend durable authority, then add backup/restore tests |
