...
| `CAP-CRM-004` | `/contacts` | `PARTIAL` | `LO`,`LV` | Phase 1 | PostgreSQL list/detail GETs and the pure create normalization/hash contract are `LOCAL-VERIFIED`; browser list/create authority is unchanged and the create API remains inactive |
...
| `CAP-CRM-105` | Product-specific contact creation | `PARTIAL` | `LO`,`LV` | Phase 1 | Generic contact schema, seeds, read API, and normalization/hash contract are proven locally, but creation remains browser-owned and no create API is registered; specialized intake remains deferred |
| `CAP-CRM-106` | Contact detail workspace and tabs | `PARTIAL` | `LO`,`LV` | Phase 1 | Exact workspace-scoped PostgreSQL detail GET exists; browser readers are not cut over and rich actions, sections, and tabs remain incomplete |
...
| `CAP-CRM-112` | Tasks board/list/create | `PARTIAL` | `LO`,`LV` | Phase 1 | Renewal follow-up completion is durable; add general task persistence |
...
| `CAP-PLAT-003` | PostgreSQL durable data/migrations | `PARTIAL` | Initial migration, deterministic renewal seed, immutable audit and repository tests are `LOCAL-VERIFIED`; completion/refetch drives six canonical surfaces while unrelated records and mutations retain browser authority | Phase 1 development-grade persistence; Phase 2 hardening | Extend durable authority, then add backup and restore tests |
...
| `CAP-PLAT-013` | Fastify Node.js TypeScript modular-monolith REST API | `PARTIAL` | Workspace-scoped renewal GET, atomic task-completion POST, and persistence readiness are `LOCAL-VERIFIED`; Foundation mode remains generation-independent and health-only | Phase 1 | Add remaining domain routes behind repositories |
