...
| `CAP-PLAT-003` | PostgreSQL durable data/migrations | `PARTIAL` | Initial migration, deterministic renewal seed, immutable audit and repository tests are `LOCAL-VERIFIED`; the workspace-scoped GET now exposes the graph, while command and browser authority remain missing | Phase 1 development-grade persistence; Phase 2 hardening | Command transactions, reload, backup and restore tests |
