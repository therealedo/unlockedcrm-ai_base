...
| Persistence | Browser `localStorage` remains UI authority; an inactive synthetic renewal graph is migrated and seeded in local PostgreSQL | Local PostgreSQL, accessed only through workspace-scoped repositories |
...
| Data access | Prisma 7.10 plus a workspace-scoped renewal repository and reviewed constraints; no CRM route consumes it yet | Prisma plus reviewed, isolated custom SQL escape hatches |
| Async work | Absent | Bounded workers behind durable job/outbox contracts |
| Deployment | `npm run dev` remains the Windows Foundation command; validated manual migration/seed commands exist, but persistent readiness does not | Unit 2C adds local persistence startup; Railway remains an unvalidated Phase 2 candidate and Vercel preview-only |
