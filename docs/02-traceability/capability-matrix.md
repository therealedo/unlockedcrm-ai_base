...
| `CAP-PLAT-003` | PostgreSQL durable data/migrations | `PARTIAL` | Initial migration, deterministic renewal seed, immutable audit and repository tests are `LOCAL-VERIFIED`; open renewals remain visible with completed or absent tasks, preserving task-history audits (`UNIT2B-FIX-2026-09-07`); no CRM route uses them | Phase 1 development-grade persistence; Phase 2 hardening | GET/command transactions, reload, backup and restore tests |
| `CAP-PLAT-004` | Workspace ownership seam / tenant isolation | `PARTIAL` | Renewal graph composite constraints and scoped repository negatives are `LOCAL-VERIFIED`; this is not tenant isolation | Phase 1 one-workspace `workspace_id` scope; Phase 3 tenant isolation | Carry centralized context through routes and later records; cross-tenant proof only in clean-room SaaS |
...
| `CAP-PLAT-008` | Audit/analytics event store | `PARTIAL` | Fixed renewal creation audit plus database update/delete rejection are `LOCAL-VERIFIED`; query and retention remain missing | Phase 1 functional local events; Phase 2 hardening | Atomic completion append, query and retention proof |
