...
| `GAP-PLAT-003` | `P0` | Durable records | `PARTIAL`, `LOCAL-VERIFIED`: renewal migration, deterministic seed, constraints and scoped repository exist; the graph drives seven GET views across six canonical renewal surfaces with reload proof; unrelated records and mutations remain browser-owned | Phase 1 | Complete mutation/refetch closure, then add backup/restore tests |
| `GAP-PLAT-004` | `P0` | Workspace ownership seam | `PARTIAL`, `LOCAL-VERIFIED`: one renewal graph has composite scope constraints, repository negatives, and server-controlled route context; hierarchy is `LIVE-VERIFIED` | Phase 1 seam; Phase 3 multi-workspace isolation | Extend the seam to all records/jobs/files/events/search/exports |
| `GAP-PLAT-005` | `P1` | Events and bounded async workers | `PARTIAL`, `LOCAL-VERIFIED`: immutable renewal creation and transactional task-completion audits exist; outbox/workers are missing | Phase 1 functional slice; Phase 2 operations | Outbox/inbox, persisted jobs, retry and reconciliation; capacity/dead-letter operations later |
...
| `GAP-PLAT-007` | `P0` | Application API | `PARTIAL`, `LOCAL-VERIFIED` | Phase 1 | Fastify health plus stable renewal GET and completion POST exist; add remaining domain routes behind repositories |
...
| `GAP-CRM-002` | `P1` | Pipeline, tasks, appointments | Local create and renewal-only managed task reads are `PARTIAL`, `LOCAL-VERIFIED` | Phase 1 | Complete lifecycle, ownership, comments, reschedule/cancel, activity events, and consistent projections |
| `GAP-CRM-003` | `P1` | Policies, renewals, dashboards, analytics | Six canonical managed read surfaces are `PARTIAL`, `LOCAL-VERIFIED`; completion UI/refetch and full policy/renewal workflows remain missing | Phase 1 | Complete the managed mutation/refetch closure, then broaden workflows without inventing provider facts |
