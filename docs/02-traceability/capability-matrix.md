...
| `CAP-PLAT-003` | PostgreSQL durable data/migrations | `PARTIAL` | Initial migration, deterministic renewal seed, immutable audit and repository tests are `LOCAL-VERIFIED`; completion-aware startup accepts only empty, exact pending, or exact completed fixed graphs, atomic completion persists across restart, and an isolated browser harness reads the seeded GET through the web proxy while UI authority remains missing | Phase 1 development-grade persistence; Phase 2 hardening | Browser-authoritative reload, backup and restore tests |
